...
class Button {
    constructor({x, y, w, h, word, touchid}) {
        this.word = word
        this.x = x
        this.y = y
        this.w = w
...
const bio = new Box({
    ...
    h: canvas.height / 12,
    x: (canvas.width / 2) - (canvas.width / 3 / 2),
    y: (canvas.height / 4) + (canvas.height / 3 / 2),
})
...
const up = new Button({
    ...
    w: canvas.width / 8,
    h: canvas.height / 15,
    x: (canvas.width / 2) - (canvas.width / 7 / 2),
    y: (canvas.height / 3.75) + (canvas.height) / 2,
    touchid: 0
})
...
const left = new Button({
    ...
    w: canvas.width / 8,
    h: canvas.height / 15,
    x: (canvas.width / 2) - (canvas.width / 2.5 / 2),
    y: (canvas.height / 3) + (canvas.height) / 2,
    touchid: 1
})
...
const right = new Button({
    ...
    w: canvas.width / 8,
    h: canvas.height / 15,
    x: (canvas.width / 2) + (canvas.width / 19),
    y: (canvas.height / 3) + (canvas.height) / 2,
    touchid: 2
})
...
const fire = new Button({
    ...
    h: canvas.height / 15,
    x: (canvas.width / 2) - (canvas.width / 7 / 2),
    y: (canvas.height / 2.5) + (canvas.height) / 2,
    touchid: 3
})
...
const buttons_pressed = [null, null, null, null]
...
document.addEventListener('touchstart', (event)=>{
    ...
    if (touch.pageX >= up.x && touch.pageX <= up.x + up.w && touch.pageY >= up.y && touch.pageY <= up.y + up.h) {
        keys.w.pressed = true
        buttons_pressed[0] = 0
        touch.identifier = 0
    }
    if (touch.pageX >= left.x && touch.pageX <= left.x + left.w && touch.pageY >= left.y && touch.pageY <= left.y + left.h) {
        keys.a.pressed = true
        buttons_pressed[0] = 1
        touch.identifier = 1
    }
    if (touch.pageX >= right.x && touch.pageX <= right.x + right.w && touch.pageY >= right.y && touch.pageY <= right.y + right.h) {
        keys.d.pressed = true
        buttons_pressed[0] = 2
        touch.identifier = 2
    }
    if (touch.pageX >= fire.x && touch.pageX <= fire.x + fire.w && touch.pageY >= fire.y && touch.pageY <= fire.y + fire.h) {
        projectiles.push(new Projectile({
            position: {
                x: (player.position.x - Math.sin(player.rotation) * -25),
                y: (player.position.y - Math.cos(player.rotation) * 25)
...
document.addEventListener('touchend', (event)=>{
    [...event.changedTouches].forEach(touch => {
        const lastTouch = document.getElementsByName(touch)
        
        lastTouch.identifier = buttons_pressed[0]
        
        if (up.touchid === lastTouch.identifier) {
            keys.w.pressed = false
            buttons_pressed[0] = null
        }
        else if (left.touchid === lastTouch.identifier) {
            keys.a.pressed = false
            buttons_pressed[0] = null
        }
        else if (right.touchid === lastTouch.identifier) {
            keys.d.pressed = false
            buttons_pressed[0] = null
        }
    })
